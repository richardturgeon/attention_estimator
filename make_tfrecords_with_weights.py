...
def convert_to_tfr(x, y, path, class_weights=None):
    ...
    with tf.io.TFRecordWriter(path) as writer:
        for i in range(x.shape[0]):
            ...
            if class_weights is not None:
                w = class_weights
                w_curr = [w[y_curr]]    # scalar value
                w_list = tf.train.FloatList(value=w_curr)
                w_feature = tf.train.Feature(float_list=w_list)
                feature_dict['class_weights'] = w_feature
...
def main(csv_path=None, outpfx=None, test=False, class_weights=False):
    """TFRecord generation."""
    ...
    convert_to_tfr(x_train, y_train, train_path, class_weights=weights)
    convert_to_tfr(x_test, y_test, test_path, class_weights=weights)            # desirable?
...
if __name__ == '__main__':
    ...
    parser.add_argument('--class_weights', default=False, action='store_true', help='apply class weights')
